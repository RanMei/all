define(["./session"], function (_session) {
	"use strict";
	...
	$(document).ready(function () {
		...
		var $$user = _session.session.getUser();
		var $$order;
		...
		function renderDI() {
			var DI = _session.session.getUser().deliveryInformation;
			console.log(DI);
		...
		function removeDI(e) {
			...
			if (confirm("确定删除该条收货信息吗？")) {
				...
				var data = JSON.stringify(_session.session.getUser().deliveryInformation[i]);
				console.log(data);
				$.ajax({
					url: "php/remove.php",
					type: "post",
		...
		function save() {
			...
			DI.DI = true;
			if (DI.consignee && /^1[3|4|5|8]\d{9}$/.test(DI.phoneNumber) && DI.deliveryAddress) {
				$.ajax({
					url: "php/insert.php",
					type: "post",
					async: false,
